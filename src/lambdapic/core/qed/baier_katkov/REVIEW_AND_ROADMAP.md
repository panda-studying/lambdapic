...
| 2026-09-17 | 记录器加场通道（`fields=True`，§4.7）；真实多 rank 生产运行验证通过（§6.4） |
...
**面向"真正可用"的排序**（2026-09-16 复核后的清单；性能类明确排除在外）：端到端验证已完成（§5.5）；剩下的是两处会静默出错的记录接口（窗口、轴）与两个"从单粒子到物理结论"的部件。
...
- **记录器**（09-17）：`fields=True` 场通道（§4.7）；真实多 rank 生产运行验证通过，含跨 rank 迁移（§6.4）。
...
### 4.7 沿任意轨迹的绝热（LCFA）参考（未决；输入通道已就绪）
...
**输入通道已于 2026-09-16 就绪**：`TrajectoryRecorder(..., fields=True)` 现在把粒子处的 $\mathbf E,\mathbf B$（SI）作为 `e`/`b` 两个数组一起落盘——正是主模拟自己 LCFA 管线读的那套共位值（`ex_part` 等），所以 $\chi$ 可以用与 PIC 完全相同的定义算出来。三条使用须知：

- 场是**半步**滞后的共位值（Yee 的固有交错，位置 $\mathbf x-\mathbf u\,\mathrm dt/2$），不是按记录位置重新插值；
- `itime == 0` 的第一个样本**没有场**（推进器还没插值过），落盘为 NaN——`t/x/u` 不受影响，消费方丢掉第 0 个场样本即可；
- 存储代价是 $\mathbf x$ 的两倍（$\mathbf E$、$\mathbf B$ 各三分量），所以默认关闭——对 BK 本身（只要 `t/x/u`）没有损失。

纯磁场记录仍有更省的等价路线（$\chi=\gamma^2\beta^2/\rho$，模块已有 `_curvature_rate`），不需要场通道。真实生产记录上已有实测：$\chi$ 中位 $2.3\times10^{-6}$、最大 $1.1\times10^{-5}$（$a_0=2$ LWFA，§6.4）。

**仍然缺的**：一个沿轨迹积分的绝热谱函数（与 BK 同一方向网格、同一 $\omega$ 网格，直接给逐 $\omega$ 的比值）。它需要一个从 SI 场到 $\chi$ 的换算（10 行，`calculate_chi_inline` 就在主模拟里，但它属于主模拟、BK 模块要保持解耦），以及系综加权（§6.3）。
...
- **`callback/trajectory.py` 的 `TrajectoryRecorder`**：挂 `start` 阶段，按 `_id` 追踪选中粒子（含跨 rank 迁移），写 NaN 填充的定形 npz。**移动窗口会在运行中新建粒子**，末态最热电子可能只有最后几百步历史——要全程记录就关掉移动窗口，或把盒子开到脉冲不出界。需要 LCFA 对比时加 `fields=True`（粒子处的 $\mathbf E,\mathbf B$，SI），见 §4.7；真实多 rank 下的验证见 §6.4。
...
- ~~**记录器的场／$\chi$ 通道**~~ **已完成 2026-09-16**：`fields=True` 落盘 `e`/`b`（SI），含 $\chi$ 与绝热参考所需的一切；非均匀场下车道正确性有单测与 MPI 用例覆盖（§4.7）。
...
### 6.4 记录器的生产级验证 ✅ 已完成（2026-09-17）

`TrajectoryRecorder` 的 MPI 正确性此前只有**合成的 6 步用例**；现在在真实生产运行上验过：`mpirun -n 2` 跑 `example/lwfa.py`（$500\times800$、300 步 $=28$ fs、无移动窗口），每个 rank 各追一个自己拥有的粒子，且 `fields=True`。

**结果**：

| 检查 | 结果 |
|---|---|
| 跨 rank 迁移 | **发生了**：粒子 0（rank0 出生）的 300 个样本里 **184 个由 rank 0、116 个由 rank 1** 记录——正是 BUG-12 当初静默毁掉的那条路径 |
| 完整性 | `n_samples = [300, 300]`，`x`/`u`/场全部有限，`t` 严格递增，无缺口 |
| 合并的正确性 | 合并后的轨迹是**物理上连续的抖动**：γ 每 ~16 步平滑地触到 ~1.000（两侧 1.03→1.14）再升到 2.35——索引错位会表现为孤立跳变，没有出现 |
| 记录的场 | $\lvert\mathbf B\rvert_{\max}=2.65\times10^4$ T $=6.0\times10^{-6}B_{\rm cr}$（$a_0=2$ 的激光磁场量级）；$\lvert\mathbf E\rvert/(c\lvert\mathbf B\rvert)$ 中位 **1.010**（激光主导处满足 EM 波关系，等离子体尾场区更高是正常的） |
| $\chi$ 反推 | 中位 $2.3\times10^{-6}$、最大 $1.1\times10^{-5}$——$a_0=2$ LWFA 的合理量级，且是**从记录本身**按主模拟的定义（`calculate_chi_inline`）算出的 |
| $\gamma$ 演化 | 1.000 → 2.349，与 §3.1 同一配置的 $\gamma_{\max}=2.42$ 一致 |

即：记录器在真实多 rank 生产运行上可用，且 §4.7 的 $\chi(t)$ 输入已经拿到真实数据。**未做**：移动窗口下的同样验证（§6.1 的陷阱与此无关，但窗口会让被追踪粒子中途消失，需要在选点上另作安排）。
