...
**(c) 一般性约束：$N\gtrsim2.7\gamma^2$。** 发射带伸展到约 $\gamma^2\omega_0$（多普勒上移分量），要求 $\omega_{max}\ge\gamma^2\omega_0$：
...
3. **实测标度（本节的落点）**：把 $a_0$ 从 2 扫到 16（其余同生产算例），每个 $a_0$ 记录最热电子，在相互作用窗口内用滑动窗量 $\omega_c=1.5\gamma^3\lvert\mathrm d\mathbf v/\mathrm dt\rvert$：
   ...
   **交叉点在 $\gamma\approx5$–10 之间**：$\lambda_0/20$ 能支撑到 $\gamma\approx5$（$a_0\approx4$），$\gamma\approx10$ 已欠分辨 5 倍、$\gamma\approx25$ 欠 43 倍。
...
**行动结论**：$\lambda_0/20$ 的生产算例可做 BK 的 $\gamma\lesssim5$ 段；LWFA 关注的 $\gamma=10$–$100$ 需要 $dx=\lambda_0/320$–$\lambda_0/2600$，**不可行**。所以 PIC 集成的落点重新确认为**弱相对论激光等离子体的红外 BK 谱**，这次是测量支撑的。

**附：磁场/回旋驱动另算。** 那里 $\rho\propto\gamma^2$ 与 $\omega_c\propto\gamma^3\Omega$ 部分相抵，要求是"每轨道约 $1.9\gamma^3\beta f$ 格"——$\gamma=10$ 约 1700 格/轨道、$dx=0.234$ 自然单位，**可行**（对比激光在同 $\gamma$ 下需 $\lambda_0/320$）。所以"高 $\gamma$ 不能做 BK"只对**激光驱动**成立；§8.3 的磁场测试台把 $dx$ 从 0.5 细化到 $\approx0.15$（每轨道约 670 格、盒子约 $750^2$、约 10 分钟一趟）即可在 $\gamma=5$ 上分辨到 $\omega_c$。
...
- **`record_adequacy` / `formation_time` 对"含长静止段"的记录给出垃圾值**：它们用 $\text{median}\lvert\mathrm d\mathbf v/\mathrm dt\rvert$，而本例 65% 的记录是静止段，中位数被 $\sim10^{-31}$ 主导，$L/\tau_f$ 报 $10^{-18}$（应为 $O(10)$–$10^2$）。**修法：只在相互作用窗口上评估**（本次检查即如此），或让 `_curvature_rate` 对静止段稳健。

**行动结论**：PIC 集成的落点是**弱相对论（$\gamma\lesssim3$–5）激光等离子体的红外 BK 谱**，而不是 LWFA 的高 $\gamma$ 谱。若目标物理需要 $\gamma\gg10$，则"BK 吃 PIC 轨迹"这条路在本分辨率下关闭，BK 只能继续作为**合成轨迹上的解析参照**（它已验证的结果都在那里）。
...
| ~~P2~~ ✅ | ~~测高 $\gamma$ 标度~~ **已完成 2026-09-16**（§8.4(d)）：$a_0$ 从 2 扫到 16，实得 **$\omega_c/\omega_0\approx0.6\gamma^{2.3}$**、分辨要求 $N\gtrsim1.6\gamma^{2.3}$。**交叉点在 $\gamma\approx5$–10**：$\lambda_0/20$ 支撑到 $\gamma\approx5$，$\gamma=10$ 欠分辨 5 倍、$\gamma=25$ 欠 43 倍。三次表述（$\gamma^2$ / $\gamma$ 无关 / $\gamma^{2.3}$）由测量裁决 | 0.5 d | §8.4 |
